...
protocol FDSTab {
    ...
    /// Optional profile image to show instead of icon (for profile tabs)
    var profileImage: String? { get }
}

extension FDSTab {
    // Default implementation - no profile image
...
struct FDSTabBarConfig {
    ...
    var inactiveIconColor: Color = Color("primaryIcon")
    ...
    var body: some View {
        ...
        ZStack(alignment: .bottom) {
            ...
            .padding(.horizontal, isFloating ? config.hPadding : 0)
                ...
                .offset(y: helper.hideTabBar ? 200 : (isFloating ? 0 : 8))
    ...
    private func tabButton(for tab: Value, at index: Int) -> some View {
        ...
        Button(action: {
            ...
            VStack(spacing: 0) {
                ...
                Group {
                    if let profileImage = tab.profileImage {
                        // Profile photo tab
                        Image(profileImage)
                            .resizable()
                            .scaledToFill()
                            ...
                            .overlay(
                                Circle()
                                    .stroke(isActive ? config.activeIconColor : config.inactiveIconColor, lineWidth: 2)
                            )
                    } else {
                        // Regular icon tab
