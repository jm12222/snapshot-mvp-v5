...
enum FDSListCellLeftAddOn {
    ...
    case asyncProfilePhoto(urlString: String, type: FDSListCellProfilePhotoType? = nil, size: FDSListCellLeftAddOnSize? = nil, placeholderIcon: String? = nil)
}
...
enum FDSListCellRightAddOn {
    ...
    case button(type: FDSButtonType, label: String, size: FDSButtonSize, action: () -> Void)
}
...
struct FDSListCell<Destination: View>: View {
    ...
    private func getLeftAddOnSize(_ addOn: FDSListCellLeftAddOn) -> CGFloat {
        ...
        case .profilePhoto(_, _, let size):
            return (size ?? defaultLeftAddOnSize).value
        case .asyncProfilePhoto(_, _, let size, _):
            return (size ?? defaultLeftAddOnSize).value
        }
    }
    ...
    private func leftAddOnView(_ addOn: FDSListCellLeftAddOn) -> some View {
        ...
        case .asyncProfilePhoto(let urlString, let type, let size, let placeholderIcon):
            asyncProfilePhotoAddOnView(urlString: urlString, type: type, size: size, placeholderIcon: placeholderIcon)
        }
    }
    ...
    @ViewBuilder
    private func asyncProfilePhotoAddOnView(urlString: String, type: FDSListCellProfilePhotoType?, size: FDSListCellLeftAddOnSize?, placeholderIcon: String?) -> some View {
        let finalSize = size ?? defaultLeftAddOnSize
        let finalType = type ?? .actor
        let finalPlaceholderIcon = placeholderIcon ?? "globe-americas-filled"
        
        FDSAsyncProfilePhoto(
            urlString: urlString,
            type: finalType,
            size: finalSize.value,
            placeholderIcon: finalPlaceholderIcon
        )
    }
    ...
    private func rightAddOnView(_ addOn: FDSListCellRightAddOn) -> some View {
        ...
        case .button(let type, let label, let size, let action):
            FDSButton(
                type: type,
                label: label,
                size: size,
...
// MARK: - Async Profile Photo Component
struct FDSAsyncProfilePhoto: View {
    let urlString: String
    let type: FDSListCellProfilePhotoType
    let size: CGFloat
                ...
                VStack(spacing: 12) {
                    ...
                    ListCellCard(title: "With left add-ons") {
                        VStack(spacing: 0) {
                            ...
                            FDSListCell(
                                headlineText: "Async profile photo (from URL)",
                                bodyText: "example.com",
                                leftAddOn: .asyncProfilePhoto(urlString: "https://example.com", type: .nonActor, size: .size40)
                            )
                        }
                    }
                    ...
                    ListCellCard(title: "With right add-ons") {
                        VStack(spacing: 0) {
                            ...
                            FDSListCell(
                                headlineText: "With toggle",
                                rightAddOn: .toggle(isOn: .constant(true)),
                                showHairline: true
                            )
                            FDSListCell(
                                headlineText: "With button",
                                rightAddOn: .button(type: .secondary, label: "Clear", size: .medium, action: {})
                            )
                        }
                    }
