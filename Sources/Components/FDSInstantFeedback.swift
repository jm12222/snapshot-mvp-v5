...
struct InstantFeedbackContainer: View {
    ...
    let leftAddOn: FDSInstantFeedbackLeftAddOn
    let onAction: (() -> Void)?
    var autoDismissDelay: TimeInterval = 5.5
    var entryDelay: TimeInterval = 0.5
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 0) {
                ...
                // - Toasts float at a top layer above ALL content, snapped to bottom of screen just above tab bar
                // - They are NOT inline in a feed — they hover over everything
                // - On screens with a bottom sheet, the toast sits above the sheet
                // - Auto-dismiss after ~4 seconds unless the user interacts
                // - On 3B-style bottom sheet screens: toast sits BELOW the dim overlay and bottom sheet

                FDSNavigationBarCentered(
                    title: "Confirmation Dialogues (Toasts)",
                    backAction: { dismiss() }
                )
