...
struct V6UXRSnapshotUnit: Identifiable, Hashable {
    let id: Int
    let title: String
    let body: String
    let image1: String
...
struct TodaysSnapshotLandingV6UXR: View {
    var onBack: (() -> Void)? = nil
    ...
    @State private var selectedUnit: V6UXRSnapshotUnit? = nil
    ...
    var body: some View {
        ...
        .navigationDestination(item: $selectedUnit) { unit in
            SnapshotUnitDetailV6UXR(unit: unit)
        }
    ...
    private func mediaCard(imageName: String, title: String, body: String) -> some View {
        ...
        ZStack(alignment: .bottom) {
            ...
            VStack(alignment: .leading, spacing: 11) { // tuned: 8 → 11 (+3pt below headline)
                ...
                WordTruncatedBodyV6UXR(text: body, foregroundColor: Color("secondaryText"))
            }
    ...
    private var v6PickedForYouUnits: [V6UXRSnapshotUnit] {
    ...
    private func groupedHighlights(units: [V6UXRSnapshotUnit]) -> some View {
        VStack(spacing: 0) {
...
//    }
    ...
    private func highlightRow(unit: V6UXRSnapshotUnit, isLast: Bool) -> some View {
        Button(action: {
            selectedUnit = unit
        }) {
            VStack(spacing: 0) {
                HStack(alignment: .top, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 10) { // tuned: 14 → 10 (-4 below headline)
                        ...
                        WordTruncatedBodyV6UXR(text: unit.body, foregroundColor: Color("secondaryText"))
                    }
                }
                .padding(.horizontal, 16)
                .padding(.vertical, 16)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        // Thesis renders without the inline accent link on
                        // the Colorado detail page — keeps the paragraph
                        // a clean primaryText block. +4 above, +8 below
                        // (4 baseline + 4 extra breathing room before bullets).
...
struct WordTruncatedBodyV6UXR: View {
    let text: String
